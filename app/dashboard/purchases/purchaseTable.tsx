...
import { StatusBadge, TableHead } from "./purchaseUi";
              ...
              purchases.map((purchase) => {
                ...
                return (
                  <tr key={purchase.id} className="transition hover:bg-slate-50">
                    ...
                    <td className="px-5 py-4 text-sm text-slate-600">{purchase.date}</td>
                    <td className="px-5 py-4">
                      <p className="font-semibold text-slate-900">{purchase.supplierName}</p>
                      {purchase.supplierPhone && (
                        <p className="mt-1 text-xs text-slate-500">{purchase.supplierPhone}</p>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      <p className="font-medium text-slate-800">{item ? item.productName : "-"}</p>
                    </td>
                    <td className="px-5 py-4">
                      {item ? (
                        <>
